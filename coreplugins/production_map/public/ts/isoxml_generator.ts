// Not used as TS, but as JS
// This file is not used in the production map plugin, just for generating
import {
  ISOXMLManager,
  ISOXMLManagerOptions,
  TAGS,
  Customer,
  Farm,
  TaskTaskStatusEnum,
  ...
  Point,
  ProductGroup,
  ProductGroupProductGroupTypeEnum,
  ValuePresentation,
  CulturalPractice,
  Product,
  ...
  CustomerAttributes,
  FarmAttributes,
  ValuePresentationAttributes,
} from "isoxml";
import { createGridParamsGenerator } from "isoxml/dist/entities/Grid/DefaultGridParamsGenerator";
import * as turf from "@turf/turf";

export interface ISOXMLInputOptions {
  geoJSONPath: string;
  bufferSize: number;
  customer: Pick<
    CustomerAttributes,
    "CustomerFirstName" | "CustomerLastName" | "CustomerCity" | "CustomerState"
  >;
  farm: Pick<FarmAttributes, "FarmDesignator" | "FarmCity" | "FarmState">;
  partfieldName: string;
  productGroupName: string;
  culturalPracticeName: string;
  productName: string;
  valuePresentation: Pick<
    ValuePresentationAttributes,
    "Offset" | "Scale" | "NumberOfDecimals" | "UnitDesignator"
  >;
  quantityDDI: number;
...
export class ISOXMLGenerator {
  private options: ISOXMLInputOptions;
  private isoxmlManager: ISOXMLManager;

  constructor(options: ISOXMLInputOptions) {
    this.options = options;
    const gridParamsGenerator = createGridParamsGenerator(1, 1);

    const managerOptions: ISOXMLManagerOptions = {
  ...
  private async loadGeoJSON(): Promise<turf.FeatureCollection<turf.Geometry>> {
    try {
      ...
      if (geoJSON.type === "FeatureCollection") {
        return geoJSON as turf.FeatureCollection<turf.Geometry>;
      } else {
  ...
  public async generate(outputPath: string = "./isoxml.zip"): Promise<void> {
    ...
    if (!hull) throw new Error("Convex hull could not be created.");
    ...
    const customer = this.isoxmlManager.createEntityFromAttributes(
      TAGS.Customer,
    ...
    const farm = this.isoxmlManager.createEntityFromAttributes(
      TAGS.Farm,
      this.options.farm
    ) as Farm;
    ...
    const isoPoints: Point[] = coords.map(
      ([lon, lat]) =>
        this.isoxmlManager.createEntityFromAttributes(TAGS.Point, {
          PointEast: lon,
          PointNorth: lat,
          PointType: "10",
        }) as Point
    ...
    const lineString = this.isoxmlManager.createEntityFromAttributes(
      TAGS.LineString,
      {
    ...
    const polygon = this.isoxmlManager.createEntityFromAttributes(
      TAGS.Polygon,
      {
    ...
    const partfield = this.isoxmlManager.createEntityFromAttributes(
      TAGS.Partfield,
      {
    ...
    const productGroup = this.isoxmlManager.createEntityFromAttributes(
      TAGS.ProductGroup,
      {
    ...
    const valuePresentation = this.isoxmlManager.createEntityFromAttributes(
      TAGS.ValuePresentation,
      this.options.valuePresentation
    ) as ValuePresentation;
    this.isoxmlManager.registerEntity(valuePresentation);
    this.isoxmlManager.rootElement.attributes.ValuePresentation = [
      valuePresentation,
    ];

    const culturalPractice = this.isoxmlManager.createEntityFromAttributes(
      TAGS.CulturalPractice,
      {
    ...
    const product = this.isoxmlManager.createEntityFromAttributes(
      TAGS.Product,
      {
        ProductDesignator: this.options.productName,
        ProductGroupIdRef:
          this.isoxmlManager.getReferenceByEntity(productGroup),
        ValuePresentationIdRef:
          this.isoxmlManager.getReferenceByEntity(valuePresentation),
        CulturalPracticeIdRef:
          this.isoxmlManager.getReferenceByEntity(culturalPractice),
